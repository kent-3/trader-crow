<script lang="ts" type="module">
  ...
  async function getBalances() {
    try {
      ...
      $scrtBalance = Number(
        parseInt(response.balance!.amount!) / 1e6
      ).toString();
      balanceX = await getBalance(
        $secretClient,
        sSCRT!,
        $secretAddress,
        $viewingKeys.get(sSCRT!.address)!
      );
      balanceY = await getBalance(
        $secretClient,
        SILK!,
        $secretAddress,
        $viewingKeys.get(SILK!.address)!
      );
    } catch (error) {
      console.log(error);
    }
...
>
  <div class="text-center space-y-2">
    ...
    <p class="font-bold font-mono text-primary-600 dark:text-primary-400">
      {$scrtBalance} SCRT
    </p>
    <p class="font-bold font-mono text-primary-600 dark:text-primary-400">
      {balanceX ?? "👀"} sSCRT
