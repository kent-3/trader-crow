...
<div
  ...
  class="flex flex-col p-6 h-full mx-auto justify-start items-left space-y-6 first:pt-8"
>
  <div class="flex flex-row items-center gap-12">
    <div class="space-y-0">
      <h2 class="font-semibold !text-4xl">Pool</h2>
      <p class="text-sm text-secondary-500-400-token">
        Provide liquidity and earn fees.
      </p>
    </div>
    <a
      href="{base}/pool/create"
      class="btn variant-ghost-secondary p-4 items-center h-3/4"
    >
      Create New Pool
    </a>
  </div>
      ...
      <tbody>
        {#each tableArr as row, i}
          <tr>
            {#if i == 0}
              <a
                class="!text-primary-500 dark:!text-success-500 font-bold uppercase"
                href="{base}/pool/{row.pool_name}"
              >
