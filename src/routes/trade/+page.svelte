<script lang="ts">
  ...
  async function checkBalanceX() {
    balanceX = await getBalance(
      $secretClient,
      selectedTokenX!,
      $secretAddress,
      $viewingKeys.get(selectedTokenX!.address)!
    );
  }
  
  async function checkBalanceY() {
    balanceY = await getBalance(
      $secretClient,
      selectedTokenY!,
      $secretAddress,
      $viewingKeys.get(selectedTokenY!.address)!
    );
  }

  // Watch for changes in selectedTokens, secretAddress, or viewingKeys and trigger checkBalance() when they change
  $: {
    if (selectedTokenX && $secretAddress && $viewingKeys) {
      checkBalanceX();
    }
  }

  $: {
    if (selectedTokenY && $secretAddress && $viewingKeys) {
      checkBalanceY();
    }
  }
...
<div
  ...
  class="flex flex-col p-4 h-full mx-auto justify-start items-center space-y-6 first:pt-8"
>
  ...
  <div
    class="card xl:w-5/12 md:w-7/12 sm:w-9/12 w-full h-auto p-8 space-y-8 items-center"
  >
    <div class="space-y-2">
      <div class="flex justify-between">
        <h2 class=" !text-base">Token X</h2>
        <button
          class="btn btn-sm py-0 px-2 hover:bg-secondary-500/20 text-ellipsis"
          >Balance: {balanceX ?? "👀"}</button
        >
      </div>
      <div class="flex justify-between space-x-2">
        <input
          bind:value={amountX}
          disabled={!swapForY}
          class="input !bg-surface-50-900-token font-heading-token text-primary-900-50-token placeholder:text-primary-900-50-token/50 font-bold"
          type="number"
          name=""
          id=""
          placeholder="0.0"
        ...
        <select
          bind:value={tokenX}
          class="select w-36 !bg-surface-50-900-token font-heading-token text-primary-900-50-token placeholder:text-primary-900-50-token/50 font-bold"
          title="Select Token"
        >
    ...
    {#if routeFound}
      <div
        class="border-2 border-primary-500-400-token !bg-surface-50-900-token"
      >
        <Accordion
          regionPanel="space-y-1 items-center"
          regionControl="font-bold"
          regionCaret="fill-primary-400"
        >
          <AccordionItem>
            ...
            <svelte:fragment slot="content">
              <div class="flex justify-between">
                ...
                <div class="font-bold text-token text-sm">1 TOKENY</div>
              </div>
              <div class="flex justify-between">
                <div class="text-sm text-primary-500-400-token">
                  Minimum Received:
                </div>
                <div class="font-bold text-token text-sm">0.99 TOKENY</div>
              </div>
              <div class="flex justify-between">
                <div class="text-sm text-primary-500-400-token">
                  Price Impact:
                </div>
                <div class="font-bold text-token text-sm">0.1%</div>
              </div>
              <div class="flex justify-between">
                <div class="text-sm text-primary-500-400-token">Fees:</div>
                <div class="font-bold text-token text-sm">0.01 TOKENX</div>
              </div>
            </svelte:fragment>
          </AccordionItem>
        </Accordion>
