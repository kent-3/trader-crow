<script lang="ts" type="module">
	...
	let sSCRT = tokenList.find((token) => token.symbol === 'sSCRT')
	let SILK = tokenList.find((token) => token.symbol === 'SILK')
	...
	async function getBalances() {
		try {
			...
			$scrtBalance = Number((parseInt(response.balance!.amount!)) / 1e6).toString()
			balanceX = await getBalance($secretClient, sSCRT!, $secretAddress, $viewingKeys.get(sSCRT!.address)!);
			balanceY = await getBalance($secretClient, SILK!, $secretAddress, $viewingKeys.get(SILK!.address)!);
...
>
	<div class="text-center space-y-2">
		...
		<p class="font-bold font-mono text-primary-600 dark:text-primary-400">{balanceX ?? '👀'} sSCRT</p>
		<p class="font-bold font-mono text-primary-600 dark:text-primary-400">{balanceY ?? '👀'} SILK</p>
		<button
